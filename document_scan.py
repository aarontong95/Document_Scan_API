...
model.to(device)
args, label_map = get_config()

def scan(file_obj):
    image = Image.open(file_obj)
    image = image.convert("RGB")
    width, height = image.size
    ocr_df = preprocess_ocr(image, width, height)
    words = list(ocr_df.text)
    actual_boxes, boxes = preprocess_box(ocr_df, width, height)
    result = predict(image, words, boxes, actual_boxes)

    return result

def preprocess_ocr(image, width, height):
...
def preprocess_box(ocr_df, width, height):
...
def predict(image, words, boxes, actual_boxes):
    ...
    for prediction, box in zip(word_level_predictions, final_boxes):
        ...
        result.append((tuple(box), predicted_label))
